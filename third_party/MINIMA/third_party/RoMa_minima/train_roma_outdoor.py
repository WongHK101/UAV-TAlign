...
def train(args):
    ...
    if e_name is not None:
        checkpoint_dir = f"{save_dir}/workspace_{e_name}/checkpoints/"
        suffix = 2
        while os.path.exists(checkpoint_dir):
            checkpoint_dir = f"{save_dir}/workspace_{e_name}_{suffix}/checkpoints/"
            suffix += 1
    else:
        checkpoint_dir = f"{save_dir}/workspace/checkpoints/"
    h, w = resolutions[resolution]
    ...
    depth_loss = RobustLosses(
        ...
        c=1e-4, )
    parameters = [
        {"params": model.encoder.parameters(), "lr": romatch.STEP_SIZE * 5e-6 * args.lr_scale / 8},
        {"params": model.decoder.parameters(), "lr": romatch.STEP_SIZE * 1e-4 * args.lr_scale / 8},
    ]
...
if __name__ == "__main__":
    ...
    parser.add_argument("--gpu_batch_size", default=3, type=int)
    parser.add_argument("--wandb_entity", required=False)
    parser.add_argument("--e_name", required=False)
