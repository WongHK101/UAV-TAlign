...
SEEDS="${SEEDS:-1 2 3}"
LOG_ROOT="${LOG_ROOT:-$OUTPUT_ROOT/prcv_s1_multiseed_${WINDOW_TAG}}"
...
run_seed() {
  local seed="$1"
  local stage_name="S1_random_selection_seed${seed}"
...
main() {
  log "Starting PRCV multi-seed S1 supplement launcher."
  ...
  for seed in $SEEDS; do
    run_seed "$seed"
    local rc="${STAGE_EXIT_CODES[S1_random_selection_seed${seed}]:-1}"
    if [[ "$rc" -ne 0 ]]; then
      append_summary "window_result=failed_seed_${seed}"
      return "$rc"
    fi
  done
  ...
  log "PRCV multi-seed S1 supplement launcher finished."
  return 0
}
